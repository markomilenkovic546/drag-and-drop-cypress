...
describe('Board management', () => {
    ...
    it('User should be able to edit a card name', () => {
        const cardName = faker.word.verb();
        board.enableEditButton(columns[0].name).click()
        board.card(columns[0].name, columns[0].cards[0].name).click()
        board.card(columns[0].name, columns[0].cards[0].name).type(cardName)
        board.disableEditButton(columns[0].name).click()
        board.card(columns[0].name, columns[0].cards[0].name + cardName).should('be.visible')
    });
});
